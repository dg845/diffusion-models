"""Script for training a diffusion model on the Fashion MNIST dataset."""
...
from diffusion_models.metrics.metrics import get_classifier_metrics
...
def train_diffusion_model(config: DDPMConfig):
    ...
    # Go from NumPy array in [0, 1] to PyTorch tensor in [-1, 1].
    transforms = Compose([
    ...
    reverse_transforms = Compose([
        ...
        # Convert to 8-bit black-and-white pixels for PIL
    ...
    for epoch in range(start_epoch, start_epoch + config.epochs):
        ...
        if (epoch + 1) % config.save_checkpoint_epochs == 0:
            ckpt_path = checkpoints_folder / f"fashion_mnist-ckpt-{epoch + 1}.pth"
